...
def make_model(args, task, eval_stage):
    ...
    if task == Task.ERM:
        if is_train:
            return ERM(args.parent_size, args.lr, args.wd)
        else:
            return ERM.load_from_checkpoint(ckpt_fpath(args, task))
    elif task == Task.VAE:
        return VAE(task, args.parent_size, args.child_size, args.h_sizes, args.y_mult, args.beta, args.prior_reg_mult,
            args.init_sd,  args.lr, args.wd)
    else:
